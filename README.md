...
[![Build Status](https://github.com/PhotogrammetryCourse/PhotogrammetryTasks2026/actions/workflows/cmake.yml/badge.svg?branch=task01&event=push)](https://github.com/PhotogrammetryCourse/PhotogrammetryTasks2026/actions/workflows/cmake.yml)

0. Сделать fork проекта (обратите внимание что задание не в ветке **master**, а в ветке **task01**)
1. [Установите OpenCV 4.11.0](https://github.com/PhotogrammetryCourse/PhotogrammetryTasks2026/blob/task01/CMakeLists.txt#L18-L35)
...
3. Отправить **Pull-request** с названием ```Task01 <ИмяНаРусском> <ФамилияНаРусском> <Аффиляция>```:

 - Скопируйте в описание [шаблон](https://raw.githubusercontent.com/PhotogrammetryCourse/PhotogrammetryTasks2026/task01/.github/pull_request_template.md)
 ...
 - Создайте PR
 - Затем дождавшись отработку Github Actions CI (около 15 минут) - скопируйте в описание PR вывод исполнения вашей программы **на CI любой OS** (через редактирование описания PR)

**Мягкий дедлайн**: 23:59 27 февраля. Мягкий дедлайн - ориентировочная рекомендация "здорово если успеете".

**Жесткий дедлайн**: 23:59 6 марта. Жесткий дедлайн - предполагается что все в него укладываются. Не доделали - зашлите хотя бы что-то. После дедлайна - штраф в баллах.
...
Реализуйте SIFT в ```src/phg/sift/sift.cpp```:
 ...
 - Выполняя **TODO** один за другим (через Ctrl+F сверху вниз) (тогда если все хорошо - **10 баллов**)
 - Тест PairMatching выдает *Final score*. Топ 3 результатов получат баллы ПРЕСТИЖА в размере +3, +2, +1.   
