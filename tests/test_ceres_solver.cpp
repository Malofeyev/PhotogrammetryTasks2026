...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Потому что ф-ция расстояния между параболойдом и точкой определяется без учёта z-координаты queryPoint 
    }
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // residual[0] = ceres::pow(line[0] * (T)samplePoint[0] + line[1] * (T)samplePoint[1] + line[2], 2.0) / (ceres::pow(line[0], 2.0) + ceres::pow(line[1], 2.0)); 
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    // -threshold < scale * line_params[d] - ideal_line[d] < threshold
    // -threshold + ideal_line[d] < scale * line_params[d] < threshold + ideal_line[d]
    // -threshold / abs(line_params[d]) + ideal_line[d] / line_params[d] < scale < threshold / abs(line_params[d]) + ideal_line[d] / line_params[d]
    
    double max_l_scale = std::numeric_limits<double>::lowest();
    double min_r_scale = std::numeric_limits<double>::max();
    const double eps = 1e-5;
    for (int d = 0; d < 3; ++d) {
        if (std::abs(line_params[d]) > eps) {
            // std::cout << "l: " << -threshold / std::abs(line_params[d]) + ideal_line[d] / line_params[d] << "  " << 
            //     "r: " << threshold / std::abs(line_params[d]) + ideal_line[d] / line_params[d] << std::endl;
            max_l_scale = std::max(max_l_scale, -threshold / std::abs(line_params[d]) + ideal_line[d] / line_params[d]);
            min_r_scale = std::min(min_r_scale, threshold / std::abs(line_params[d]) + ideal_line[d] / line_params[d]);
        }
    }
    // std::cout << "max_l: " << max_l_scale << " min_r: " << min_r_scale << std::endl;
    double scale = (max_l_scale + min_r_scale) / 2;
    ...
    if (outliers_fraction == 0 || use_huber) {
        for (int d = 0; d < 3; ++d) {
            ...
            //  домножить на коэффицент a,b,c, чтобы были ближе к идеальным 
            ...
            //  например, нормализовать a, b, c и выразить a=sqrt(1-b^2) в формуле прямой 
    ...
    ASSERT_GT(inliers_fraction, 0.99 * (1.0 - outliers_fraction)); // TODO 8 раскоментируйте, почему эта проверка падает? как поправить? 
                                                                   // Потому что доля inliers зависит от outliers_fraction
